...
For the SPARQL demonstrator we load only **`out.ttl`** (the instance data
generated by R2RML). Because R2RML emits an explicit `rdf:type` for every
individual via `rr:class`, the SPARQL queries used in the demonstrator do not
require OWL inference and can run against the raw ABox alone. The OWL ontology
itself is published separately at the GitHub Pages namespace declared above
and only needs to be loaded into Fuseki if a client requires OWL-based
inference (subclass propagation, domain/range entailment).
...
```powershell
Invoke-WebRequest -Uri "http://localhost:3030/audiovisual/data" -Method POST `
    -InFile "out.ttl" -ContentType "text/turtle" -UseBasicParsing
```

**Verify Data Loaded:**
- Go to http://localhost:3030 → Click "audiovisual" → "info"
- Should show ~14,000 triples
- Run a sanity check in the Fuseki SPARQL endpoint:
  ```sparql
  PREFIX ont: <https://khalilankri.github.io/audiovisual-ontology/1.0#>
  SELECT (COUNT(*) AS ?n) WHERE { ?t a ont:Title }
  ```
  This should return 174 (the number of Title individuals).

### Optional: Convert the OWL to Turtle (for RDFLib-based tools)

Protégé saves the ontology in **OWL/XML**, which is fine for Fuseki / Apache
Jena but is **not** understood by RDFLib (and therefore not by pyshacl). If
you want to validate the data with pyshacl while passing the ontology as the
`-e` (extra graph) source, or if you simply want to inspect the ontology in
Turtle syntax, run the conversion helper:
...
This produces `audiovisual-ontology.ttl` next to the `.owl` file. The Apache
Jena toolchain used in the main validation workflow (`shacl validate ...`)
does **not** need this conversion — Jena reads OWL/XML natively.
